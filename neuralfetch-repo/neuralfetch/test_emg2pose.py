...
from neuralfetch.studies.emg2pose import BidsEmg2pose, Emg2pose
...
def test_emg2pose_study_source(tmp_path):
    ...
    assert events["type"].tolist() == ["BidsEmg2pose"]
...
def test_bids_reader_restores_emg_and_joint_channel_semantics(monkeypatch, tmp_path):
    """The BDF conversion restores native EMG scale and radian joint labels."""
    channel_names = [*Emg2pose.EMG_CHANNEL_NAMES, *Emg2pose.JOINT_CHANNEL_NAMES]
    raw = mne.io.RawArray(
        np.concatenate(
            (
                np.full((Emg2pose.EMG_CHANNEL_COUNT, 1), 5e-6),
                np.full((Emg2pose.POSE_CHANNEL_COUNT, 1), 0.5),
            )
        ),
        mne.create_info(
            channel_names,
    ...
    # ``read_raw_bdf`` stores BDF physical units here even when BIDS lacks
    # ``channels.tsv`` and MNE initially assigns every channel the EEG type.
    raw._orig_units = {name: "µV" for name in channel_names}
    monkeypatch.setattr(
