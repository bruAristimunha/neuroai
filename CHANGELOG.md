...
- `neuralfetch`: raised the MOABB requirement to `>=1.6.1` and removed the workarounds it makes redundant — the `BNCI2020_002` reshape override, the `MartinezCagigal2023` `stim_trial` monkeypatch, the MAMEM Figshare listing cache (all fixed in MOABB #1068), the `BNCI2025_001` non-EEG channel retyping and the `Cattan2019_PHMD` montage (both fixed in MOABB #1161), plus a dead `Stieger2021Continuous._get_dataset` override. Note MOABB 1.6.0 also converts `MartinezCagigal2023Checker`/`Pary` EEG from microvolts to volts, and `Lee2019_*` now exposes both sessions instead of one (`num_timelines` doubled for the three `Lee2019Eeg*` studies; delete any existing `timelines.csv` for them so it is rebuilt).
