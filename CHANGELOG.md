...
- `neuralbench`: `Data.drop_nonfinite_target_segments` drops windows whose target is not finite, and `emg pose` enables it — emg2pose's `skip_ik_failures`, so IK failures remove the window from every split rather than being masked out frame by frame (#229).
